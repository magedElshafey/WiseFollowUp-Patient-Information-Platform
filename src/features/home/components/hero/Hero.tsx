...
//               </p>
//               {/* quick suggestions */}
//               {query?.data && query?.data?.featured_words?.length > 0 && (
...
const HomeHero: React.FC = () => {
  ...
  return (
    <HeroLayout aria-labelledby="home-hero-heading">
      <FetchHandler queryResult={query} skeletonType="hero">
        <div className="mx-auto max-w-5xl min-h-[200px]  rounded-card bg-bg-surface/70 backdrop-blur-sm p-6 md:p-8 shadow-soft">
          {query.data?.is_active && (
            <>
              <HeroTextBlock
                title={query.data.heading}
